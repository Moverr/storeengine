...
object chartapp {
  ...
  object ChatRoom {
    ...
    def session(
               ...
               ): Behavior[SessionCommand] = Behaviors.receiveMessage { msg =>
      msg match {
        case PostMessage(message) =>
          room ! PublishMessage(screenName ,  message )
          Behaviors.same
  ...
  object Main {
    import kodeinc.chartapp.ChatRoom.Gabller
    def apply(): Behavior[NotUsed] = Behaviors.setup { ctx =>
      ctx.log.info("The Applicatin Main Has Started")

      val chartRoom = ctx.spawn(ChatRoom(),"chartroom")
      val gabllerRef = ctx.spawn(Gabller(),"Gabbler")
      ctx.watch(gabllerRef)
      chartRoom !  ChatRoom.GetSession("ol’ Gabbler", gabllerRef)
